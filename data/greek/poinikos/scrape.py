...
PDF_FILE = "Ποινικός-Κώδικας.pdf"
TXT_OUTPUT = "poinikos_kodikas_clean.txt"

# Regex για αναγνώριση του πρώτου άρθρου
...
def extract_useful_text(pdf_path, txt_path, start_regex):
    ...
    # Καθαρισμός γραμμών από περιττά δεδομένα
    cleaned_lines = []
    for line in useful_text.splitlines():
        line = line.strip()
        if (
            line and
            not re.match(r"^Ημ.*?:", line) and
            not re.match(r"^ΦΕΚ", line, flags=re.IGNORECASE) and
            not line.lower().startswith("σχόλια") and
            not line.lower().startswith("περιγραφή όρου") and
            len(line) > 3
        ):
