...
INPUT_HTML = "universal.html"  # το αποθηκευμένο αρχείο HTML
...
def save_links_to_txt(links, out_file):
    ...
    with open(out_file, "a", encoding="utf-8") as f:  # άλλαξε από "w" σε "a"
        for title, url in links:
            f.write(f"{title}\n{url}\n\n")
    print(f"💾 Appended {len(links)} links to {out_file}")


def main():
    links = extract_links_from_html(INPUT_HTML)
    save_links_to_txt(links, os.path.join(OUT_DIR, "universal_instruments_list.txt"))
